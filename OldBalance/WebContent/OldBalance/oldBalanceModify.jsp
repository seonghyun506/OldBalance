		...
		<div id='modify'>
			...
			<form action="<%=pjName%>/Modify?cmd=modify-member" method='post'>
				...
				<input type="hidden" name="id" value="<%=member.getCustId() %>"/>
					<pre>
						<p>기존 비밀번호 </p><input class="put form-control" type="text" name="password" /><input type="hidden" name="hiddenpwd" value="<%=member.getCustPw() %>" />
					</pre><%} %>
					<pre><p>새 비밀변호  </p><input type="text" class="put form-control" name="newpwd" /></pre>
					<pre><p>비밀번호 확인 </p> <input class="put form-control" type="text" /></pre>
				</div>
				<hr />
				<ul class=post_check>
